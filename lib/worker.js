...
Worker.prototype.runServer = function (config) {
    ...
    var proxyErrorHandler = function (err, req, res) {
        ...
        if (req.retries >= config.retryOnError) {
            if (err.code === 'ECONNREFUSED') {
                this.cache.isBackendSleeping(req.meta.virtualHost, function (err, isSleeping) {
                    // TODO worry about err
                    if (isSleeping) {
                        var sleepMessage = function(res) {
                            var filePath = rootDir + '/static/sleep_page.html';
                        ...
                        var onError = function (error, stdoutbuf, stderrbuf) {
                            console.log('out:');
                            console.log(stdoutbuf.toString());
                            console.log('err:');
                            console.log(stderrbuf.toString());
                            return sleepMessage(res);
                        };
                        var onSuccess = function (stdoutbuf, stderrbuf) {
                            // this undoes the previous incremental effect up above.
                            // NOTE that the wake script changes redis state
                              // so that the backend is no longer recognized as sleeping, and this wont happen more than once (theoretically...)
                            // TODO FIX the wake script to wait for the backend to fully go up. this isn't fully reliable.
                            setTimeout(function() {
                              req.retries = 0;
                              req.emit('retry');
                            }, 1500);
                            // now when retry is emitted, it will seriously retry,
                              // rather than going into this branch which is the max-retries-exceeded branch.
                        };
                        require('./wake').wake(req.meta.backendHost, req.meta.virtualHost, onSuccess, onError);
                    }
                });
                return;
            } else {
