...
var legacyConfigSupport = function (config) {
    if (config.redisHost || config.redisPort) {
        this.log('WARNING: old redis syntax is deprecated! Please migrate to ' +
...
/*
 * This method does takes a callback which has the boolean of whether or not the backend is sleeping.
 */
Cache.prototype.isBackendSleeping = function (backendId, callback) {
    this.log('Checking if ' + backendId + ' is sleeping.');
    this.redisClient.get('sleeping:' + backendId, function(err, reply) {
        callback(reply === '1');
    });
}
